...
module.exports = function RandomEvent(indexPrefix) {
    ...
    event.current_user_last_login_date = date.setHours(date.getHours() - 4);
    //event.referer = 'http://' + samples.referrers() + '/' + samples.tags() + '/' + samples.astronauts();
    event.current_user = samples.currentUser();
    event.device_ip = event.ip;
    //event.bytes = event.response < 500 ? samples.lessRandomRespSize(event.extension) : 0;
    event.cpu_usage = (Math.random() * (0 - 100) + 100).toFixed(2);
    event.process_cpu_usage = (Math.random() * (2 - 40) + 40).toFixed(2);

    event.ram_usage = (Math.random() * (0 - 8) + 8).toFixed(2);
    event.process_ram_usage = (Math.random() * (2 - 6) + 6).toFixed(2);

    event.bw_out_usage = (Math.random() * (0 - 10) + 10).toFixed(2);
    event.bw_in_usage = (Math.random() * (0 - 50) + 50).toFixed(2);

    event.user_time_load = (Math.random() * (5 - 90) + 90).toFixed(2);

    event.total_ram = samples.randomRam();
    ...
    event.host = 'theacademyofperformingartsandscience.org';
    event.request = '/people/type:astronauts/name:' + samples.astronauts() + '/profile';
    event.url_data = 'https://' + event.host + event.request;
